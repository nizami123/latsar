...
class Masuk extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model(['Masuk_model','Komoditas_model']);
    }

    public function index() {
        $data['masuk'] = $this->Masuk_model->getAll();
        $data['wilayah']  = $this->Komoditas_model->getKecamatan();
        $data['komoditas'] = $this->Komoditas_model->getKomoditasMasuk();
        $data['title'] = "Data Masuk";
        $data['filename'] = "data_masuk";
        $data['js'] = 'masuk.js';
    ...
    public function update() {
        ...
        $this->Masuk_model->update($post['id_masuk'], $data);
        redirect('masuk');
    }
    ...
    public function download_template() {
        if ($this->session->userdata('jabatan') == 'Admin Kecamatan') {
            ...
            $header = 'DATA TERNAK MASUK KECAMATAN ' . strtoupper($kec->nama_wilayah);
        } else {
            $wilayah = $this->Komoditas_model->getKecamatan();
            $header = 'DATA TERNAK MASUK KABUPATEN LAMONGAN';
        }
        $komoditas = $this->Komoditas_model->getKomoditasMasuk();

        require APPPATH . 'third_party/PHPExcel/Classes/PHPExcel.php';
        $objPHPExcel = new PHPExcel();
        $sheet = $objPHPExcel->setActiveSheetIndex(0);
        $sheet->setTitle('Masuk');
        ...
        header('Content-Disposition: attachment;filename="template_masuk.xlsx"');
    ...
    public function upload_excel() {
        ...
        $upload_path = FCPATH . 'uploads/masuk/';
    ...
    {
        $ids = $this->input->post('id_masuk');
        if($ids) {
            $this->db->where_in('id_masuk', $ids);
            $this->db->delete('trx_masuk');
        }
        redirect('masuk');
    }
