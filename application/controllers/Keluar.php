...
class Keluar extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model(['Keluar_model','Komoditas_model']);
    }

    public function index() {
        $data['keluar'] = $this->Keluar_model->getAll();
        $data['wilayah']  = $this->Komoditas_model->getKecamatan();
        $data['komoditas'] = $this->Komoditas_model->getKomoditasKeluar();
        $data['title'] = "Data Keluar";
        $data['filename'] = "data_keluar";
        $data['js'] = 'keluar.js';
    ...
    public function update() {
        ...
        $this->Keluar_model->update($post['id_keluar'], $data);
        redirect('keluar');
    }
    ...
    public function download_template() {
        if ($this->session->userdata('jabatan') == 'Admin Kecamatan') {
            ...
            $header = 'DATA TERNAK KELUAR KECAMATAN ' . strtoupper($kec->nama_wilayah);
        } else {
            $wilayah = $this->Komoditas_model->getKecamatan();
            $header = 'DATA TERNAK KELUAR KABUPATEN LAMONGAN';
        }
        $komoditas = $this->Komoditas_model->getKomoditasKeluar();

        require APPPATH . 'third_party/PHPExcel/Classes/PHPExcel.php';
        $objPHPExcel = new PHPExcel();
        $sheet = $objPHPExcel->setActiveSheetIndex(0);
        $sheet->setTitle('Keluar');
        ...
        header('Content-Disposition: attachment;filename="template_keluar.xlsx"');
    ...
    {
        $ids = $this->input->post('id_keluar');
        if($ids) {
            $this->db->where_in('id_keluar', $ids);
