  ...
  <section class="content-header">
    <div class="container-fluid d-flex justify-content-between align-items-center">
      ...
      <div>
        ...
        <a href="<?= site_url('rtp/download_template') ?>" class="btn btn-success btn-sm">Download Template</a>
      </div>
    </div>
  </section>
      ...
      <div class="card">
        <div class="card-body">
          <table id="example1" class="table table-bordered table-striped align-middle">
            ...
            <tbody>
              <?php foreach($rtp as $p): ?>
                <?php if ($p->jumlah == 0) continue; ?>
              <tr>
                <td style="text-align: center;">
                  <div class="custom-checkbox">
                    <input type="checkbox" class="checkItem" id="cb<?= $p->id_rtp ?>" value="<?= $p->id_rtp ?>">
                    <label for="cb<?= $p->id_rtp ?>"></label>
                  </div>
                </td>
                ...
                <td>
                  <button 
                    class="btn btn-warning btn-sm btnEditRtp" 
                    data-id="<?= $p->id_rtp ?>" 
                    data-bulan="<?= $p->bulan ?>" 
                    data-tahun="<?= $p->tahun ?>" 
                  ...
                  <a href="<?= site_url('rtp/delete/'.$p->id_rtp) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Hapus data ini?')">Hapus</a>
                </td>
              </tr>
              <?php endforeach; ?>
            </tbody>
...
<div class="modal fade" id="addModal">
  <div class="modal-dialog">
    <form action="<?= site_url('rtp/save') ?>" method="post">
      <div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Tambah Rtp</h5></div>
...
<div class="modal fade" id="editModal">
  <div class="modal-dialog">
    <form action="<?= site_url('rtp/update') ?>" method="post">
      <div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Edit Rtp</h5></div>
        <div class="modal-body">
          <input type="hidden" name="id_rtp" id="edit_id">
...
<div class="modal fade" id="uploadModal">
  <div class="modal-dialog">
    <form action="<?= site_url('rtp/upload_excel') ?>" method="post" enctype="multipart/form-data">
      <div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Upload Excel Rtp</h5></div>
        <div class="modal-body">
