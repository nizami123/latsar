  ...
  <section class="content-header">
    <div class="container-fluid d-flex justify-content-between align-items-center">
      ...
      <div>
        ...
        <a href="<?= site_url('kelahiran/download_template') ?>" class="btn btn-success btn-sm">Download Template</a>
      </div>
    </div>
  </section>
      ...
      <div class="card">
        <div class="card-body">
          <table id="example1" class="table table-bordered table-striped align-middle">
            ...
            <tbody>
              <?php foreach($kelahiran as $p): ?>
              <tr>
                <td style="text-align: center;">
                  <div class="custom-checkbox">
                    <input type="checkbox" class="checkItem" id="cb<?= $p->id_kelahiran ?>" value="<?= $p->id_kelahiran ?>">
                    <label for="cb<?= $p->id_kelahiran ?>"></label>
                  </div>
                </td>
                ...
                <td>
                  <button 
                    class="btn btn-warning btn-sm btnEditKelahiran" 
                    data-id="<?= $p->id_kelahiran ?>" 
                    data-bulan="<?= $p->bulan ?>" 
                    data-tahun="<?= $p->tahun ?>" 
                  ...
                  <a href="<?= site_url('kelahiran/delete/'.$p->id_kelahiran) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Hapus data ini?')">Hapus</a>
                </td>
              </tr>
              <?php endforeach; ?>
            </tbody>
...
<div class="modal fade" id="addModal">
  <div class="modal-dialog">
    <form action="<?= site_url('kelahiran/save') ?>" method="post">
      <div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Tambah Kelahiran</h5></div>
...
<div class="modal fade" id="editModal">
  <div class="modal-dialog">
    <form action="<?= site_url('kelahiran/update') ?>" method="post">
      <div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Edit Kelahiran</h5></div>
        <div class="modal-body">
...
<div class="modal fade" id="uploadModal">
  <div class="modal-dialog">
    <form action="<?= site_url('kelahiran/upload_excel') ?>" method="post" enctype="multipart/form-data">
      <div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Upload Excel Kelahiran</h5></div>
        <div class="modal-body">
          <div class="form-group">
            <label>Bulan</label>
            <?= bulan_dropdown('bulan') ?>
